...
namespace cgCourse {
    std::future<float*> RayCaster::start(const Camera &cam, const unsigned int &samples, int startY, int endY){
        return std::async(RayCaster::trace, cam, scene, width, height, samples, startY, endY);
    }
    
    float* RayCaster::trace(const Camera &cam, std::shared_ptr<Scene> scene, int width, int height, const unsigned int &samples, int startY, int endY){
